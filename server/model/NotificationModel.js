...
class NotificationModel {
    static async getAllNotifications() {
        const [results] = await pool.query(`
            SELECT nr.id, n.notification_id, n.subject, n.body, n.functionality, e.username, e.employee_id, n.notify_date, nr.mark_read
            FROM notification n
    ...
    static async getNotificationsById(id) {
        const [results] = await pool.query(`
            SELECT nr.id n.notification_id, n.subject, n.body, n.functionality, nr.employee_id, e.username, n.notify_date
            FROM notification n
            JOIN notification_recipients nr
                ON n.notification_id = nr.notification_id
            JOIN employees e
    ...
    static async newNotification(notification) {
        ...
        const functionalityJson = JSON.stringify(notification.functionality)
        console.log(functionalityJson)
        const [results] =  await pool.query(`
            INSERT INTO notification (subject, body, functionality) VALUES (?, ?, ?)
        `, [notification.subject, notification.body, functionalityJson])
