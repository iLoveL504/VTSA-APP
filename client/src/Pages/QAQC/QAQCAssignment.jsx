...
const QAQCAssignment = () => {
    ...
    const filteredProjects = useMemo(() => {
        ...
        if (activeView === 'pending') {
            filtered = filtered.filter(p => p.qaqc_pending);
        } else if (activeView === 'assigned') {
            filtered = filtered.filter(p => p.qaqc_is_assigned);
        } else if (activeView === 'ongoing') {
            filtered = filtered.filter(p => p.qaqc_ongoing);
        }
    ...
    const handleAssignConfirm = async () => {
        ...
        try {
            ...
            const endpoint = `/api/projects/qaqc/assign/${assignModal.project.id}`;
            const method = 'PUT';
    ...
    const stats = {
        ...
        pending: projects?.filter(p => p.qaqc_pending).length || 0,
        assigned: projects?.filter(p => p.qaqc_is_assigned).length || 0,
        ongoing: projects?.filter(p => p.qaqc_ongoing).length || 0
    };
    ...
    return (
        <div className="qaqc-modern-dashboard">
                        ...
                        <div className="modal-content">
                            ...
                            <div className="project-overview-card">
                                ...
                                <div className={`status-tag ${
                                    assignModal.project?.qaqc_ongoing ? 'ongoing' : 
                                    assignModal.project?.qaqc_is_assigned ? 'assigned' : 
                                    'pending'
                                }`}>
                                    {assignModal.project?.qaqc_ongoing ? 'Ongoing' : 
                                     assignModal.project?.qaqc_is_assigned ? 'Assigned' : 'Pending'}
                                </div>
                            </div>
                            ...
                            <div className="form-section">
                                <div className="section-header">
                                    ...
                                    <div>
                                        ...
                                        <p>Choose from available QAQC technicians</p>
                                    </div>
                                </div>
            ...
            <div className="dashboard-layout">
                ...
                <aside className={`sidebar ${sidebarCollapsed ? 'collapsed' : ''}`}>
                    <div className="sidebar-header">
                        <DashboardIcon />
                        {!sidebarCollapsed && <span>QAQC Menu</span>}
                        <button 
                        ...
                        {!sidebarCollapsed && (
                            <div className="team-widget">
                                ...
                                <div className="team-list">
                                    {qaqcEmployees.slice(0, 4).map(tech => {
                                        const assignedProjects = qaqcTechs?.filter(q => q.employee_id === tech.employee_id && q.project_id) || [];
                                        return (
                                            <div key={tech.employee_id} className="team-member">
                                                ...
                                                <div className="member-info">
                                                    ...
                                                    <span className="member-projects">{assignedProjects.length} projects</span>
                                                </div>
                                            </div>
                                        );
                                    })}
                                ...
                                >
                                    <div className="project-header">
                                        ...
                                        <div className={`status-badge ${
                                            project.qaqc_ongoing ? 'ongoing' : 
                                            project.qaqc_is_assigned ? 'assigned' : 
                                            'pending'
                                        }`}>
                                            {project.qaqc_ongoing ? 'Ongoing' : 
                                             project.qaqc_is_assigned ? 'Assigned' : 'Pending'}
                                        </div>
                                    </div>
                                    ...
                                    <div className="project-details">
                                        ...
                                        <div className="detail">
                                            ...
                                            <span>
                                                {project.qaqc_inspection_date 
                                                    ? new Date(project.qaqc_inspection_date).toLocaleDateString('en-GB')
                                                    : 'Not scheduled'
                                                }
                                            </span>
                                        </div>
                                        <div className="detail">
                                            <InfoIcon />
                                            <span>{project.qaqc_inspection_reason || 'Routine Inspection'}</span>
                                        </div>
                                    </div>

                                    <div className="project-actions">
                                        {project.qaqc_pending ? (
                                            <button 
                                        ...
                                        {(project.qaqc_is_assigned || project.qaqc_ongoing) ? (
                                            <button 
                                                className="btn-edit"
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                            ...
                                            <div className="assigned-tech-card">
                                                ...
                                                <div className="tech-details">
                                                    ...
                                                    <span className="assignment-date">
                                                        Assigned on: {assignedTech.qaqc_inspection_date ? 
                                                            new Date(assignedTech.qaqc_inspection_date).toLocaleDateString('en-GB') : 'N/A'}
                                                    </span>
                                                </div>
                                            </div>
                                        ) : (
